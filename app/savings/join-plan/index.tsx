...
import { joinSavingsPlan } from "../../../services/savingsService";
...
export default function JoinPlanScreen() {
    ...
    const handleConfirmJoin = async () => {
        if (!plan || !plan.savingsCircleId) {
            Alert.alert(
                "Error",
                "This plan is missing a Circle ID and cannot be joined."
            );
            return;
        }
        if (pin.length !== 4) {
        ...
        setIsJoining(true);
        const response = await joinSavingsPlan(plan.savingsCircleId);
        setIsJoining(false);
